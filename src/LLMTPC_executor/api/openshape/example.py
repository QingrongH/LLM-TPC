...
def load_ply_dense(file_name, num_points=10000, y_up=True):
    pcd = o3d.io.read_point_cloud(file_name)
    xyz = np.asarray(pcd.points)
    rgb = np.asarray(pcd.colors)
    n = xyz.shape[0]
...
model = load_model(config, model_name='OpenShape/openshape-pointbert-vitg14-rgb')
model.eval()

print("loading OpenCLIP model...")
open_clip_model, _, open_clip_preprocess = open_clip.create_model_and_transforms('ViT-bigG-14', pretrained='laion2b_s39b_b160k', cache_dir="OpenShape_code")
open_clip_model.cuda().eval()

print("extracting 3D shape feature...")
xyz, feat = load_ply_dense("demo/owl.ply")
